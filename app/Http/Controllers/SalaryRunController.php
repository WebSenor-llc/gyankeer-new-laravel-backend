...
class SalaryRunController extends Controller
{
    ...
    public function generate(Request $req)
    {
        ...
        //   • With a salary_group_id → narrow to that group.
        //   • Without one → preview ALL active employees of the company.
        $employees = collect();
        $previewLoaded = false;
        if ($req->boolean('get_list') && $companyId) {
            $empQ = \App\Models\Employee::with(['department','salary_group'])
                ->where('active_flag', true)
                ->where('company_id', $companyId);
            if ($salaryGroupId) {
                $empQ->where('salary_group_id', $salaryGroupId);
            }
            ...
            // When previewing all groups, sort by group then emp_id so the
            // table reads cleanly contractor-by-contractor.
            if ($salaryGroupId) {
                $empQ->orderBy('emp_id');
            } else {
                $empQ->orderBy('salary_group_id')->orderBy('emp_id');
            }
            $employees = $empQ->get();
            $previewLoaded = true;
        }
