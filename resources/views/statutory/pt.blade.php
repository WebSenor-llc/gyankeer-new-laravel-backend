...
<div class="p-4">
    ...
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
        <h1 class="text-xl font-bold">Professional Tax</h1>
        <div class="flex gap-2 flex-wrap">
            <form method="POST" action="{{ route('statutory.pt.generate') }}" class="inline">
                @csrf
                <input type="hidden" name="year"            value="{{ $year }}">
                <input type="hidden" name="month"           value="{{ $month }}">
                <input type="hidden" name="salary_group_id" value="{{ $salaryGroupId }}">
                <button class="tb-btn primary">Generate PT for this period</button>
            </form>
            <a href="{{ route('statutory.pt.pdf', ['year' => $year, 'month' => $month, 'salary_group_id' => $salaryGroupId]) }}"
               target="_blank"
               class="tb-btn primary"
               style="background:#DC2626;border-color:#B91C1C">⬇ PDF</a>
            <a href="{{ route('statutory.pt.pdf', ['year' => $year, 'month' => $month, 'salary_group_id' => $salaryGroupId, 'format' => 'csv']) }}"
               class="tb-btn primary"
               style="background:#16A34A;border-color:#15803D">⬇ CSV</a>
            <a href="{{ route('statutory.pt.pdf', ['year' => $year, 'month' => $month, 'salary_group_id' => $salaryGroupId, 'format' => 'xls']) }}"
               class="tb-btn primary"
    ...
    <div class="card p-4 mb-4 flex items-center justify-between" style="background:#FEF2F2;border-color:#FCA5A5">
        <div><div class="text-[11px] text-red-700 uppercase font-semibold">Total PT for Period</div>
            <div class="text-2xl font-bold text-red-700">&#8377;{{ number_format($total, 0) }}</div></div>
        <div class="text-xs text-slate-500">{{ \DateTime::createFromFormat('!m', $month)->format('F') }} {{ $year }} • {{ $rows->count() }} employees</div>
    </div>

    <div class="card overflow-x-auto">
        <table class="grid-tbl">
            <thead><tr><th>Emp ID</th><th>Employee</th><th>State</th><th>Slab Applied</th><th>PT Amount</th><th>Status</th></tr></thead>
            <tbody>
                @forelse($rows as $r)
                    <tr>
                        ...
                        <td>&#8377;{{ number_format($r->pt_amount ?? 0, 0) }}</td>
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-6 text-slate-500">
                        No PT records for this period{{ $salaryGroupId ? ' in selected group' : '' }}. Records are generated when payroll is run.
                    </td></tr>
                @endforelse
            </tbody>
        </table>
